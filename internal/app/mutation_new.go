...
func (s *WorkflowService) NewIssue(ctx context.Context,o NewIssueOptions)(result.CommandResult,int){
	...
	issue,err:=s.GitHub.CreateIssue(ctx,repo,o.Title,body);if err!=nil{
		if existing,conflict,e2:=find();e2==nil&&existing!=nil&&!conflict{return createdIssueResult(repo,*existing,preview,"UNKNOWN_OUTCOME_RECOVERED")};if conflict{return workflowFail("new","CONFLICT","create operation_id was persisted with different task contract",map[string]any{"operation_id":op},preview)}
