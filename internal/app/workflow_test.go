...
func workflowComment(t *testing.T, digest string) domain.GitHubComment {
	t.Helper(); branch := "main"
	e := domain.WorkflowEvent{Schema: domain.WorkflowEventSchemaV1, EventID: "e1", ParentEventID: nil, OperationID: "o1", RunID: "r1", EventType: domain.EventStart, OccurredAt: "2026-09-15T04:00:00Z", Repository: "o/r", Issue: 7, ContractDigest: digest, StateBefore: domain.StateReady, StateAfter: domain.StateInProgress, Git: &domain.EventGitSnapshot{Branch: &branch, Head: "abc", ChangedPaths: []string{}}, Data: map[string]any{}}
	raw, _ := json.Marshal(e); return domain.GitHubComment{ID: 1, CreatedAt: e.OccurredAt, Body: "[START]\n\n<!-- iw:workflow-event:v1\n" + string(raw) + "\n-->"}
}
...
func TestWorkflowStatusSuccess(t *testing.T) {
	...
	r, code := s.Status(context.Background(), WorkflowOptions{CWD: "/repo", Issue: 7}); if code != 0 || !r.OK { t.Fatalf("%d %#v", code, r) }
	report := r.Data.(WorkflowReport); if report.Aggregate.State != domain.StateInProgress || report.ContractChanged || report.ProtocolSource != domain.WorkflowEventSchemaV1 { t.Fatalf("%#v", report) }
	if len(r.Warnings) != 1 || r.Warnings[0] != "WORKFLOW_V1_COMPAT" { t.Fatalf("%v", r.Warnings) }
}
func TestWorkflowStatusReportsContractChangedWithoutFailing(t *testing.T) {
	old := contractBody(); changed := strings.Replace(old, "\nG\n", "\nG changed\n", 1); s := newService(changed, []domain.GitHubComment{workflowComment(t, domain.ContractDigest(old))})
	r, code := s.Status(context.Background(), WorkflowOptions{CWD: "/repo", Issue: 7}); if code != 0 || !r.OK { t.Fatalf("%d %#v", code, r) }
	report := r.Data.(WorkflowReport); if !report.ContractChanged { t.Fatal("expected contract changed") }; if len(r.Warnings) == 0 || r.Warnings[0] != "CONTRACT_CHANGED" { t.Fatalf("%v", r.Warnings) }
}
func TestResumeDryRunRejectsReady(t *testing.T) {
	s := newService(contractBody(), nil); r, code := s.ResumeDryRun(context.Background(), WorkflowOptions{CWD: "/repo", Issue: 7}); if code != 5 || r.Error == nil || r.Error.Code != "TRANSITION_BLOCKED" { t.Fatalf("%d %#v", code, r) }
}
func TestExplicitRepoMismatchFailsClosed(t *testing.T) {
	s := newService(contractBody(), nil); r, code := s.Status(context.Background(), WorkflowOptions{CWD: "/repo", Repo: "x/y", Issue: 7}); if code != 3 || r.Error == nil || r.Error.Code != "REPOSITORY_IDENTITY_MISMATCH" { t.Fatalf("%d %#v", code, r) }
}
...
func TestDoneRecoveryDoesNotReuseOldNext(t *testing.T) {
	...
	start := domain.WorkflowEvent{Schema: domain.WorkflowEventSchemaV1, EventID: "e1", OperationID: "o1", RunID: "r1", EventType: domain.EventStart, OccurredAt: "2026-09-15T04:00:00Z", Repository: "o/r", Issue: 7, ContractDigest: digest, StateBefore: domain.StateReady, StateAfter: domain.StateInProgress, Git: &domain.EventGitSnapshot{Branch: &branch, Head: "abc", ChangedPaths: []string{}}, Data: map[string]any{}}
	p1 := "e1"; cp := domain.WorkflowEvent{Schema: domain.WorkflowEventSchemaV1, EventID: "e2", ParentEventID: &p1, OperationID: "o2", RunID: "r1", EventType: domain.EventCheckpoint, OccurredAt: "2026-09-15T04:01:00Z", Repository: "o/r", Issue: 7, ContractDigest: digest, StateBefore: domain.StateInProgress, StateAfter: domain.StateInProgress, Git: &domain.EventGitSnapshot{Branch: &branch, Head: "abc", ChangedPaths: []string{}}, Data: map[string]any{"summary": "x", "next": "review", "validation": []any{}}}
	p2 := "e2"; final := domain.WorkflowEvent{Schema: domain.WorkflowEventSchemaV1, EventID: "e3", ParentEventID: &p2, OperationID: "o3", RunID: "r1", EventType: domain.EventFinal, OccurredAt: "2026-09-15T04:02:00Z", Repository: "o/r", Issue: 7, ContractDigest: digest, StateBefore: domain.StateInProgress, StateAfter: domain.StateDone, Git: &domain.EventGitSnapshot{Branch: &branch, Head: "abc", ChangedPaths: []string{}}, Data: map[string]any{"summary": "done", "validation": []any{}, "delivery": map[string]any{"commit": "abc", "pushed": false, "pr": nil}}}
	comments := []domain.GitHubComment{}; for i, e := range []domain.WorkflowEvent{start, cp, final} { raw, _ := domain.RenderWorkflowEvent(e); comments = append(comments, domain.GitHubComment{ID: int64(i + 1), Body: raw, CreatedAt: e.OccurredAt}) }
	s := newService(body, comments); r, c := s.Status(context.Background(), WorkflowOptions{CWD: "/repo", Issue: 7}); if c != 0 || !r.OK { t.Fatalf("%d %#v", c, r) }
	recovery := r.Data.(WorkflowReport).Recovery; if len(recovery.SuggestedActions) != 0 { t.Fatalf("%v", recovery.SuggestedActions) }; if recovery.LastNext != "" { t.Fatalf("COMPLETED recovery must not expose stale next: %q", recovery.LastNext) }
}
