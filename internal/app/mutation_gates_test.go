...
func TestScopeProposalBlocksFinalUntilRejected(t *testing.T) {
	...
	r, c := s.Scope(context.Background(), p)
	if c != 0 || !r.OK { t.Fatalf("propose %d %#v", c, r) }
	rep := r.Data.(WorkflowReport)
	if len(rep.Aggregate.UnresolvedScopeProposals) != 1 { t.Fatalf("%+v", rep.Aggregate.UnresolvedScopeProposals) }
	proposal := rep.Aggregate.UnresolvedScopeProposals[0]
	fn := mo("scope-final"); fn.Summary = "done"
	if r, c := s.Final(context.Background(), fn); c != 5 || r.Error == nil || r.Error.Code != "SCOPE_UNRESOLVED" { t.Fatalf("final %d %#v", c, r) }
	...
	if r, c := s.Final(context.Background(), fn); c != 0 || !r.OK || r.State != "DONE" { t.Fatalf("final after reject %d %#v", c, r) }
}
...
func TestBlockedMustResumeBeforeFinal(t *testing.T) {
	s, _, _ := stateService(t, false)
	...
	if r, c := s.Final(context.Background(), fn); c != 5 || r.Error == nil || r.Error.Code != "TRANSITION_BLOCKED" { t.Fatalf("final %d %#v", c, r) }
