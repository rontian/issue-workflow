...
func RenderTaskContract(in TaskContractInput) (string, error) {
	if strings.TrimSpace(in.ContractID) == "" || strings.TrimSpace(in.Goal) == "" || len(in.Scope) == 0 || len(in.OutOfScope) == 0 || len(in.AcceptanceCriteria) == 0 {
		return "", NewWorkflowError("INVALID_INVOCATION", "Goal/Scope/Out of Scope/Acceptance Criteria and contract_id are required")
	}
	meta, _ := json.Marshal(map[string]string{"schema": TaskContractSchema, "contract_id": in.ContractID})
...
func OperationFingerprint(t WorkflowEventType, data map[string]any) string {
	raw, _ := json.Marshal(map[string]any{"event_type": t, "data": data})
