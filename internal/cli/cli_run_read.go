...
func(c *CLI)runEnvironmentOrRead(ctx context.Context,command string,o options,wo app.WorkflowOptions,cwd string)(bool,int){
	...
	case "restore":if o.Apply&&o.DryRun{return true,c.render(result.Failure(command,"INVALID_INVOCATION","--apply and --dry-run cannot be used together",nil,nil),result.ExitInvalid,o.JSON)};if c.Restore==nil{return true,c.render(result.Failure(command,"ENVIRONMENT_INVALID","restore service unavailable",nil,nil),result.ExitEnvironment,o.JSON)};r,code:=c.Restore.Run(ctx,app.RestoreOptions{WorkflowOptions:wo,Apply:o.Apply});return true,c.render(r,code,o.JSON)
	...
	case "new":r,code:=c.Workflow.NewIssue(ctx,app.NewIssueOptions{WorkflowOptions:wo,DryRun:o.DryRun,OperationID:o.OperationID,Title:o.Title,Goal:o.Goal,Scope:o.ScopeItems,OutOfScope:o.OutOfScope,Constraints:o.Constraints,Acceptance:o.Acceptance,Validation:o.Validation,Dependencies:o.Dependencies,ContractID:o.ContractID});return true,c.render(r,code,o.JSON)
